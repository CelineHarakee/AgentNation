...
def _score_scenario(result: ScenarioResult, priority: OptimizationPriority) -> float:
    ...
    Higher = better. Weights shift based on policymaker's stated priority.
    """
...
def _build_rag_query(sector: str, policy_goal: str, priority: OptimizationPriority) -> str:
    """Build a retrieval query targeting policy benchmarks and comparison data."""
    return (
        f"{sector} sector workforce policy comparison benchmarks "
        f"implementation strategies {priority.value} risk assessment "
        f"Saudi Arabia Vision 2030 {policy_goal}"
    )
...
def run_caa(
    ...
    """
    Comparative Assessment Agent (CAA) — Scores + RAG-enriched narrative

    Scores all scenarios quantitatively, ranks them, then calls Gemini with
    RAG context (OECD benchmarks, Saudi sector data) to produce a grounded
    comparison narrative and conditions statement.
    """

    # ── RAG retrieval ─────────────────────────────────────────────────────────
    rag_context = ""
    if is_available():
        query  = _build_rag_query(sector, policy_goal, priority)
        chunks = retrieve(query, agent="CAA", n_results=5, sector=sector)
        rag_context = format_context(chunks, max_chars=2500)
        if rag_context:
            logger.info(f"[CAA] RAG retrieved {len(chunks)} chunks")
        else:
            logger.info("[CAA] RAG returned no relevant chunks.")
    else:
        logger.info("[CAA] RAG not available.")
    ...
    # ── LLM narrative (RAG-enriched) ─────────────────────────────────────────
    scenarios_summary = "\n".join([
        f"Scenario {r.spec.scenario_id} — {r.spec.name}:\n"
        f"  Strategy: {r.spec.rationale}\n"
        f"  Time: {r.spec.time_horizon} | Budget: {r.spec.budget_limit*100:.0f}% | "
        f"  Training: {r.spec.training_investment}\n"
        f"  Risk: {r.maa_output.severity_score:.1f}/100 ({r.maa_output.risk_classification.value})\n"
    ...
    if rag_context:
        context_section = f"""
RETRIEVED BENCHMARKS AND CONTEXT (use to ground your analysis in real-world evidence):
{rag_context}

---
"""

    prompt = f"""You are a policy comparison analyst writing a briefing for a government committee.
{context_section}
...
Write the following — plain paragraphs only, no bullet points, no headers.

1. COMPARISON_NARRATIVE (3-4 sentences): Explain what differentiates the scenarios and why
the ranking came out this way. Reference benchmarks or precedents from the retrieved context
where relevant. Be specific about which metrics drive the differences.

2. CONDITIONS_STATEMENT (1-2 sentences): Describe what conditions would cause the ranking
to change — e.g. if budget increased, or if a shorter timeline became mandatory.
...
{{
  ...
  "conditions_statement": "<1-2 sentence statement>"
    ...
    if not comparison_narrative:
        comparison_narrative = (
            f"Scenario {best_id} scored highest ({best_score:.1f}/100) under the "
            f"'{priority.value}' priority. The ranking reflects differences in risk "
            f"severity, budget utilization, and training pipeline adequacy."
        )
    if not conditions_statement:
        conditions_statement = (
            "The ranking would shift if budget constraints were relaxed or if "
            "the implementation timeline became more flexible."
        )
    ...
    explanation = (
        ...
        f"{'A hybrid scenario is recommended given limitations across all options.' if hybrid_recommended else 'The recommended scenario is feasible as designed.'}"
    )

    logger.info(
        f"[CAA] Ranking complete — best: Scenario {best_id} ({best_score:.1f}) | "
        f"hybrid_needed: {hybrid_recommended} | rag={'yes' if rag_context else 'no'}"
    )
