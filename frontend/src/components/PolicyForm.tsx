          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              <input
                type="number"
                min={0} max={100}
                value={Math.round(form.budget_limit * 100)}
                onChange={e => setForm(f => ({ ...f, budget_limit: Math.min(100, Math.max(0, Number(e.target.value))) / 100 }))}
                disabled={isLocked}
                className="w-10 text-right font-mono text-xs text-accent-blue bg-transparent border-none p-0 focus:outline-none focus:ring-0 [&::-webkit-inner-spin-button]:opacity-0 hover:[&::-webkit-inner-spin-button]:opacity-100"
              />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              <input
                type="number"
                min={0} max={100}
                value={Math.round(form.workforce_capacity * 100)}
                onChange={e => setForm(f => ({ ...f, workforce_capacity: Math.min(100, Math.max(0, Number(e.target.value))) / 100 }))}
                disabled={isLocked}
                className="w-10 text-right font-mono text-xs text-accent-blue bg-transparent border-none p-0 focus:outline-none focus:ring-0 [&::-webkit-inner-spin-button]:opacity-0 hover:[&::-webkit-inner-spin-button]:opacity-100"
              />
              <span className="font-mono text-xs text-text-muted">%</span>
